            ...
            <div class="card-body">
                ...
                <div style="overflow-x:auto;">
                    <table id="example" class="table table-striped table-bordered dt-responsive" cellspacing="0"
                        width="100%">
                        <a href="{{ route('penggeledahans.create') }}" class="btn btn-md btn-success mb-5 btn-block"><i
                            ...
                            @forelse ($Penggeledahans as $penggeledahan)
                                <tr>
                                    ...
                                    <td class="text-center">
                                        ...
                                        {{-- <a href="https://wa.me/?text=Laporan Penggeledahan - Tanggal: {{ $penggeledahan->tanggal }} - {{ asset('/storage/penggeledahans/' . $penggeledahan->image_1) }}"
                                            target="_blank">
                                            <img src="path/to/whatsapp-icon.png" alt="Bagikan ke WhatsApp">
                                        </a> --}}


                                        <form onsubmit="return confirm('Apakah Anda Yakin ?');"
                                            action="{{ route('penggeledahans.destroy', $penggeledahan->id) }}"
                                            method="POST">
                                            <a href="{{ route('penggeledahans.previewPdf', $penggeledahan->id) }}"
                                                class="btn btn-sm btn-primary">
                                                <i class="fas fa-eye"></i> <!-- FontAwesome icon for 'show' -->
                                            </a>
                                            <a href="{{ route('penggeledahans.exportPdf', $penggeledahan->id) }}"
                                            ...
                                            @if (Auth::check() && (Auth::user()->is_admin == 1 || \Carbon\Carbon::now()->format('Y-m-d') == $penggeledahan->tanggal))
                                                <a href="{{ route('penggeledahans.edit', $penggeledahan->id) }}"
                                                    class="btn btn-sm btn-success">
                                                    <i class="fas fa-edit"></i> <!-- Ikon FontAwesome untuk 'show' -->
                                                </a>
                                                <button type="submit" class="btn btn-sm btn-danger">
                                                    <i class="fas fa-trash-alt"></i>
                                                    <!-- Ikon FontAwesome untuk 'delete' -->
                                                </button>
                                            @endif
